...
impl Device {
    ...
    pub fn is_input(&self) -> bool {
        self.device_type == DeviceType::InputDevice
    }

    pub fn is_output(&self) -> bool {
        self.device_type == DeviceType::OutputDevice
    }
    
    pub fn is_duplex(&self) -> bool {
        self.device_type == DeviceType::DuplexDevice
    }
}
...
impl DeviceTrait for Device {
    ...
    fn supported_duplex_configs(
        ...
        let config = match self.default_duplex_config() {
            Err(_) => None,
            Ok(f) => {
                Some(SupportedDuplexStreamConfigRange {
    ...
    {
        if self.device_type != DeviceType::InputDevice {
    ...
    where
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        if self.device_type != DeviceType::OutputDevice {
    ...
    where
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        if self.device_type != DeviceType::DuplexDevice {
        ...
        let mut stream = Stream::new_duplex(client, conf.input_channels, conf.output_channels, data_callback, error_callback);
